import { TIPO_DOCUMENTO_DRIVE_LABELS } from "@/types/documentos-drive";
...
import { DocumentoDriveWithProcess } from "@/hooks/useAllDocumentosDrive";
import { Link } from "react-router-dom";

interface DriveDocumentoCardWithProcessProps {
  documento: DocumentoDriveWithProcess;
  onEdit: (documento: DocumentoDriveWithProcess) => void;
  onDelete: (id: string) => void;
}

export function DriveDocumentoCardWithProcess({ 
  documento, 
  onEdit, 
  onDelete 
}: DriveDocumentoCardWithProcessProps) {
    ...
    <Card className="p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          {documento.processo && (
            <Link 
              to={`/dashboard/processos?id=${documento.processo.id}`}
              className="flex items-center gap-1 text-sm text-primary hover:underline mb-2"
            >
              <FileText className="h-3 w-3" />
              {documento.processo.numero_processo || `Processo ${documento.processo.tipo}`}
            </Link>
          )}
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon" aria-label="Abrir ações do documento">
