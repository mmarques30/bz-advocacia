...
// Busca histórico de uma conversa direto na Z-API e insere em mensagens_sdr
// as mensagens (humanas e do lead) que não chegaram via webhook.
// Resolve o gap quando a Z-API para de entregar webhooks fromMe=true.
...
function extrairConteudo(m: ZapiMessage): { texto: string | null; tipoMidia: string | null } {
  ...
  if (textoDireto && textoDireto.trim()) {
    return { texto: textoDireto.trim(), tipoMidia: null };
  }
...
export async function syncLead(
  ...
  // IDs e timestamps já registrados (para dedup eficiente)
  ...
  // Se humano falou via celular e o bot não estava pausado, pausa.
...
Deno.serve(async (req) => {
  ...
  try {
    const res = await syncLead(sb, body.lead_id, limit);
